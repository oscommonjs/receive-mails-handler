...
# @Modify From: http://blog.csdn.net/marksinoberg
...
def get_mail_details(msg):
    ...
    # 匹配版权信息
    regx = re.compile(config.DOMAIN_REGX)
    # 处理版权链接
    for link in regx.findall(content):
        print(link)
        os.system('python ex_sql.py u '+link)
...
if __name__ == '__main__':
    ...
    while not exitcode:
        ...
        for index, mail in enumerate(mails):
            print("["+str(index)+": "+decode_byte(mail.status)+"]")
...
mails = get_parsed_msg(config.EMAIL_ACCOUNT, config.EMAIL_PASSWORD, 3, 1)
#data = mails[1].data
